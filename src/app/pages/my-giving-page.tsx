...
import { Lock, TrendingUp, Calendar, DollarSign, Heart, MapPin, Download, Settings, CreditCard, ArrowUpRight } from "lucide-react";
              ...
              <div className="rounded-xl bg-white p-6 shadow-sm">
                ...
                <div className="space-y-4">
                  {monthlyPledges.map((pledge) => (
                    <div key={pledge.id} className="rounded-lg border border-gray-200 p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="mb-1 font-medium text-gray-900">{pledge.campaignName}</div>
                          <div className="flex items-center gap-4 text-sm text-gray-600">
                            <span>${pledge.amount}/month</span>
                            <span>•</span>
                            <span>Next payment: {new Date(pledge.nextPayment).toLocaleDateString()}</span>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="mb-1 text-lg font-bold text-teal-600">${pledge.totalDonated}</div>
                          <div className="text-xs text-gray-500">Total donated</div>
                        </div>
                      </div>
                      <div className="mt-3 flex gap-2">
                        <button className="rounded px-3 py-1 text-xs font-medium text-gray-700 hover:bg-gray-100">
                          Pause
                        </button>
                        <button className="rounded px-3 py-1 text-xs font-medium text-gray-700 hover:bg-gray-100">
                          Edit Amount
                        </button>
                        <button className="rounded px-3 py-1 text-xs font-medium text-red-600 hover:bg-red-50">
                          Cancel
                        </button>
                      </div>
                    </div>
